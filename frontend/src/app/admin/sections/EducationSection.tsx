    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
      <div className="mb-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {editingEducationId && (
            <button type="button" onClick={onCancelEducationEdit} disabled={submitting} className="rounded-xl border border-white/10 bg-slate-900/80 px-3 py-2 text-sm text-slate-200 disabled:opacity-60">
              Cancel edit
            </button>
          )}
          <button type="button" onClick={onSaveEducation} disabled={submitting} className="rounded-xl border border-white/10 bg-slate-900/80 px-3 py-2 text-sm text-slate-200 disabled:opacity-60">
            {submitting ? "Saving..." : editingEducationId ? "Update education" : "Create education"}
          </button>
        </div>
      </div>
          ...
          {education.map((item) => (
            <div key={item.id} className="rounded-2xl border border-white/10 bg-slate-900/70 p-4">
              <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => onEditEducation(item)} disabled={submitting} className="rounded-lg border border-indigo-300/40 bg-indigo-500/10 px-2.5 py-1 text-xs font-semibold text-indigo-200 disabled:opacity-60">Edit</button>
      ...
      <div className="mt-5 rounded-2xl border border-white/10 bg-slate-900/70 p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {editingCertificationId && (
              <button type="button" onClick={onCancelCertificationEdit} disabled={submitting} className="rounded-xl border border-white/10 bg-slate-800 px-3 py-2 text-sm text-slate-200 disabled:opacity-60">
                Cancel edit
              </button>
            )}
            <button type="button" onClick={onSaveCertification} disabled={submitting} className="rounded-xl border border-white/10 bg-slate-800 px-3 py-2 text-sm text-slate-200 disabled:opacity-60">
              {submitting ? "Saving..." : editingCertificationId ? "Update certification" : "Create certification"}
            </button>
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="mt-4 space-y-3">
          ...
          {certifications.map((item) => (
            <div key={item.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-slate-900/80 px-3 py-2">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => onEditCertification(item)} disabled={submitting} className="rounded-lg border border-indigo-300/40 bg-indigo-500/10 px-2.5 py-1 text-xs font-semibold text-indigo-200 disabled:opacity-60">Edit</button>
