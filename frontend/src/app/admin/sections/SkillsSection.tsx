    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block text-sm text-slate-300">
          Category
          <input value={draftSkill.category} onChange={(event) => setDraftSkill((current) => ({ ...current, category: event.target.value }))} placeholder="Frontend" className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3 py-2.5 text-white" />
        </label>
        <label className="block text-sm text-slate-300">
          Skill name
          ...
          {skills.map((skill) => (
            <div key={skill.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-slate-900/70 px-3 py-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => onEdit(skill)} disabled={submitting} className="rounded-lg border border-indigo-300/40 bg-indigo-500/10 px-2.5 py-1 text-xs font-semibold text-indigo-200 disabled:opacity-60">Edit</button>
